...
from src.cli.theme import console, C, D, G, M, R, Y, OK, FAIL, WARN
from src.cli.commands.files import (
    CodeBlock, WriteRecord, dedup_files, extract_blocks, is_identical,
    view_diff, write_file,
)
...
def cmd_set_provider(prov: str, provider_holder: list[str | None],
                     model_holder: list[str | None]) -> None:
    if not prov:
...
def cmd_gemini(query: str, history: list[BaseMessage],
    ...
    from src.core.gemini_cli import (
        ...
        query_streaming, build_context_prompt,
    )
    ...
    full_prompt = build_context_prompt(
        query, codebase_context=codebase_ctx,
        memory=memory, history_summary=history_summary,
    )
...
def cmd_gemini_interactive() -> None:
    ...
    from src.core.gemini_cli import (
        is_gemini_cli_installed, get_install_instructions, launch_interactive,
    )
    if not is_gemini_cli_installed():
        console.print(f"  {FAIL} Gemini CLI not found on PATH\n")
        console.print(f"  {D}{get_install_instructions()}{R}\n")
